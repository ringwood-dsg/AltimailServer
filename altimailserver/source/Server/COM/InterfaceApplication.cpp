...
STDMETHODIMP InterfaceApplication::Start()
{
   try
   {
      ...
      if (!HM::Application::Instance()->StartServers())
         return COMError::GenerateError("Server start failed. Please check the Altimail Server error log.");
...
STDMETHODIMP InterfaceApplication::SubmitEMail()
{
   try
   {
      ...
      pServiceManager.UserControlService("AltimailServer", SubmitEmailOpCode);
...
InterfaceApplication::EnsureDatabaseConnectivity_()
{
   ...
   {
      return COMError::GenerateError("The connection to the database is not available. Please check the Altimail Server error log for details.");
   }
