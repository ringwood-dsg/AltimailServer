...
STDMETHODIMP InterfaceUtilities::RetrieveMessageID(BSTR sFilename, hyper *messageID)
{
   try
   {
      ...
      *messageID = id;
      ...
      if (result == false)
         return COMError::GenerateError("Retrieval of message-ID failed. Please check Altimail Server error log.");
...
STDMETHODIMP InterfaceUtilities::PerformMaintenance(eMaintenanceOperation operation)
{
   try
   {
      ...
      if (result == false)
         return COMError::GenerateError("The maintenance operation failed. Please see Altimail Server log for details.");
