...
namespace HM
{
   ...
   SpamProtection::AddSpamScoreHeaders(std::shared_ptr<Message> pMessage, std::set<std::shared_ptr<SpamTestResult> > setResult, bool classifiedAsSpam)
   {
      ...
      if (classifiedAsSpam)
      {
         if (config.GetAddHeaderSpam())
         {
            pMessageData->SetFieldValue("X-AltimailServer-Spam", "YES");
         }
      ...
      if (config.GetAddHeaderReason())
      {
         ...
         for (; iter != iterEnd; iter++, iFieldIdx++)
         {
            ...
            pMessageData->SetFieldValue("X-AltimailServer-Reason-" + StringParser::IntToString(iFieldIdx), sHeaderValue);
         }

         pMessageData->SetFieldValue("X-AltimailServer-Reason-Score", StringParser::IntToString(iTotalScore));
      }
