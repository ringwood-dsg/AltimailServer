...
namespace HM
{
   ...
   Events::FireOnClientValidatePassword(std::shared_ptr<const Account> pAccount, const String &sPassword)
   {
      ...
      pContainer->AddObject("ALTIMAILSERVER_ACCOUNT", pPersistentAccount, ScriptObject::OTAccount);
      ...
      if (sScriptLanguage == _T("VBScript"))
      {
         ...
         sEventCaller.Format(_T("OnClientValidatePassword(ALTIMAILSERVER_ACCOUNT, \"%s\")"), sEscapedPassword.c_str());
      }
   ...
   Events::FireOnDeliveryStart(std::shared_ptr<Message> pMessage)
   {
      ...
      std::shared_ptr<ScriptObjectContainer> pContainer = std::shared_ptr<ScriptObjectContainer>(new ScriptObjectContainer);
      std::shared_ptr<Result> pResult = std::shared_ptr<Result>(new Result);
      pContainer->AddObject("ALTIMAILSERVER_MESSAGE", pMessage, ScriptObject::OTMessage);
      pContainer->AddObject("Result", pResult, ScriptObject::OTResult);
      String sEventCaller = "OnDeliveryStart(ALTIMAILSERVER_MESSAGE)";
   ...
   //---------------------------------------------------------------------------()
   {
      if (Configuration::Instance()->GetUseScriptServer())
      {
         ...
         pContainer->AddObject("ALTIMAILSERVER_MESSAGE", pMessage, ScriptObject::OTMessage);
         pContainer->AddObject("Result", pResult, ScriptObject::OTResult);
         String sEventCaller = "OnDeliverMessage(ALTIMAILSERVER_MESSAGE)";
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      if (Configuration::Instance()->GetUseScriptServer())
      {
         ...
         if (sScriptLanguage == _T("VBScript"))
         {
            ...
            sEventCaller.Format(_T("OnDeliveryFailed(ALTIMAILSERVER_MESSAGE, \"%s\", \"%s\")"), sRecipientCopy.c_str(), sErrorMessageCopy.c_str());
         }
         else if (sScriptLanguage == _T("JScript"))
         {
            sRecipientCopy.Replace(_T("'"), _T("\\'"));
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      if (sScriptLanguage == _T("VBScript"))
      {
         ...
         if (pMessage)
            sEventCaller.Format(_T("OnExternalAccountDownload(ALTIMAILSERVER_FETCHACCOUNT, ALTIMAILSERVER_MESSAGE, \"%s\")"), sRemoteUIDCopy.c_str());
         else
            sEventCaller.Format(_T("OnExternalAccountDownload(ALTIMAILSERVER_FETCHACCOUNT, Nothing, \"%s\")"), sRemoteUIDCopy.c_str());
      }
      else if (sScriptLanguage == _T("JScript"))
      {
         sRemoteUIDCopy.Replace(_T("'"), _T("\\'"));
         ...
         if (pMessage)
            ...
            sEventCaller.Format(_T("OnExternalAccountDownload(ALTIMAILSERVER_FETCHACCOUNT, null, '%s')"), sRemoteUIDCopy.c_str());
      }

      std::shared_ptr<ScriptObjectContainer> pContainer  = std::shared_ptr<ScriptObjectContainer>(new ScriptObjectContainer);
      
      pContainer->AddObject("ALTIMAILSERVER_FETCHACCOUNT", fetchAccount, ScriptObject::OTFetchAccount);
