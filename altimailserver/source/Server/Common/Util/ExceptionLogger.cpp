...
namespace HM
{
   ...
   ExceptionLogger::Log(int exception_code, EXCEPTION_POINTERS* pExp)
   {
      ...
      String command_line = "AltimailServer.Minidump.exe";

      unsigned int exit_code;
      ProcessLauncher launcher(command_line);
      if (!launcher.Launch(exit_code))
      {
         ErrorManager::Instance()->ReportError(ErrorManager::Critical, 5521, "StackLogger::Log", "An error has been detected. Altimail Server was unable to launch minidump generator.");
         return;
      }
      ...
      {
         String message = Formatter::Format("An error has been detected. Altimail Server attempted to generate minidump, but AltimailServer.Minidump.exe returned {0}.", exit_code);
