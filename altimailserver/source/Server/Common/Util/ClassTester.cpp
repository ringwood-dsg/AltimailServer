...
namespace HM
{
   ...
   ClassTester::DoTests()
   {
      ...
      OutputDebugString(_T("AltimailServer: Testing mime parser\n"));
	   MimeTester *pMimeTester = new MimeTester;
      pMimeTester->TestFolder("C:\\Temp\\Testdata\\martin");
	   delete pMimeTester;

      OutputDebugString(_T("AltimailServer: Testing StringParser\n"));
      StringParserTester *pParser = new StringParserTester();
      pParser->Test();
      delete pParser;
  
      OutputDebugString(_T("AltimailServer: Testing FileUtilities\n"));
      FileUtilitiesTester fileUtilitiesTester;
      fileUtilitiesTester.Test();

      OutputDebugString(_T("AltimailServer: Testing MessageUtilities\n"));
      MessageUtilitiesTester messageUtilitiesTester;
      messageUtilitiesTester.Test();

      OutputDebugString(_T("AltimailServer: Testing Formatter\n"));
      FormatterTester formatterTester;
      formatterTester.Test();

      OutputDebugString(_T("AltimailServer: Testing TimeoutCalculator\n"));
      TimeoutCalculatorTester timeoutCalculatortester;
      timeoutCalculatortester.Test();

      OutputDebugString(_T("AltimailServer: Test DateTime\n"));
      DateTimeTests tests;
      tests.Test();

      OutputDebugString(_T("AltimailServer: Testing Utilities\n"));
      UtilitiesTester *pTestUtilities = new UtilitiesTester();
      pTestUtilities->Test();
      delete pTestUtilities;

      OutputDebugString(_T("AltimailServer: Test BLChecktester\n"));
      BLCheckTester blchecktester;
      blchecktester.Test();

      OutputDebugString(_T("AltimailServer: Testing SPF\n"));
      SPFTester *pSPF = new SPFTester();
      pSPF->Test();
      delete pSPF;

      OutputDebugString(_T("AltimailServer: Testing SHA256\n"));
      HashCreatorTester tester;
      tester.Test();


      OutputDebugString(_T("AltimailServer: Testing RegularExpressionTester\n"));
      RegularExpressionTester *pRegExTest = new RegularExpressionTester();
      pRegExTest->Test();
      delete pRegExTest;

      OutputDebugString(_T("AltimailServer: Testing Base64\n"));
      ...
      OutputDebugString(_T("AltimailServer: Testing AddresslistParser\n"));
      AddresslistParserTester *pTest2 = new AddresslistParserTester();
      pTest2->Test();
      delete pTest2;

      OutputDebugString(_T("AltimailServer: Testing charset\n"));
      CharsetTester *pCharsetTester = new CharsetTester;
      pCharsetTester->Test();
      delete pCharsetTester;


      OutputDebugString(_T("AltimailServer: Testing LocalIPAddresses\n"));
      LocalIPAddressesTester *pTest4 = new LocalIPAddressesTester();
      pTest4->Test();
      delete pTest4;

      OutputDebugString(_T("AltimailServer: Testing MessageData\n"));
      MessageDataTester *pMsgData = new MessageDataTester();
      pMsgData->Test();
      delete pMsgData;

      OutputDebugString(_T("AltimailServer: Testing Time\n"));
      ...
      OutputDebugString(_T("AltimailServer: Testing BlowFishEncryptorTester\n"));
      BlowFishEncryptorTester *pTest3 = new BlowFishEncryptorTester();
      pTest3->Test();
      delete pTest3;

      OutputDebugString(_T("AltimailServer: Testing IMAPSimpleCommandParserTester\n"));
