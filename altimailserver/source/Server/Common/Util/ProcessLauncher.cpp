...
namespace HM
{
   ...
   ProcessLauncher::Launch(unsigned int &exitCode)
   {
      ...
      if (error_log_timeout_ > 0)
      {
         ...
         case WAIT_TIMEOUT:
            {
               String errorMessage = Formatter::Format("A launched process did not exit within an expected time. The command line is {0}. The timeout occurred after {1} milliseconds. Altimail Server will continue to wait for process to finish.", command_line_, error_log_timeout_);
