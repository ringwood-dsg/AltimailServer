...
namespace HM
{
   Application::Application() :
      ...
      unique_id_(0)
   {
      version_ = Formatter::Format("{0}-B{1}", ALTIMAILSERVER_VERSION, ALTIMAILSERVER_BUILD);
   ...
   {
#if _WIN64
      return "x64"; //We should, from now on, only be in x64 mode.
#else
      return "x86";
#endif 
   }
   ...
   Application::OpenDatabase(String &sErrorMessage)
   {
      ...
      {
         ErrorManager::Instance()->ReportError(ErrorManager::Critical, 4354, "Application::OpenDatabase", "Altimail Server failed to connect to the database server. Error message: " + sErrorMessage);
         return false;
      }
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      HMODULE handle = GetModuleHandle(_T("altimailserver.exe"));
