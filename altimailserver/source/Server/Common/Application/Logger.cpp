...
namespace HM
{
   ...
   Logger::GetCurrentLogFileName(LogType lt) 
   {
      ...
      {
      case Normal:
         sFilename.Format(_T("%s\\altimailserver_%s.log"), log_dir_.c_str(), theTime.c_str());
         break;
      case Error:
         sFilename.Format(_T("%s\\ERROR_altimailserver_%s.log"), log_dir_.c_str(), theTime.c_str());
         break;
      case AWStats:
         sFilename.Format(_T("%s\\altimailserver_awstats.log"), log_dir_.c_str());
         break;
      case Backup:
         sFilename.Format(_T("%s\\altimailserver_backup.log"), log_dir_.c_str());
         break;
      case Events:
         sFilename.Format(_T("%s\\altimailserver_events.log"), log_dir_.c_str());
         break;
      case IMAP:
         if (sep_svc_logs_) 
            sFilename.Format(_T("%s\\altimailserver_IMAP_%s.log"), log_dir_.c_str(), theTime.c_str());
         else
            sFilename.Format(_T("%s\\altimailserver_%s.log"), log_dir_.c_str(), theTime.c_str());
         break;
      case POP3:
         if (sep_svc_logs_) 
            sFilename.Format(_T("%s\\altimailserver_POP3_%s.log"), log_dir_.c_str(), theTime.c_str());
         else
            sFilename.Format(_T("%s\\altimailserver_%s.log"), log_dir_.c_str(), theTime.c_str());
         break;
      case SMTP:
         if (sep_svc_logs_) 
            sFilename.Format(_T("%s\\altimailserver_SMTP_%s.log"), log_dir_.c_str(), theTime.c_str());
         else
            sFilename.Format(_T("%s\\altimailserver_%s.log"), log_dir_.c_str(), theTime.c_str());
         break;
      }
