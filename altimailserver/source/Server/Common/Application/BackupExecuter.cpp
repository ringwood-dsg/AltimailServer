...
namespace HM
{
   ...
   BackupExecuter::StartBackup()
   {
      ...
      // file
      String sZipFile;
      sZipFile.Format(_T("%s\\ASBackup %s.7z"), destination_.c_str(), sTime.c_str());

      String sXMLFile;
      sXMLFile.Format(_T("%s\\AltimailServerBackup.xml"), destination_.c_str());
   ...
   BackupExecuter::BackupDataDirectory_(const String &sDataBackupDir)
   {
      ...
      if (!bResult)
      {
         Logger::Instance()->LogBackup("Failed to delete files in backup root directory. Please see Altimail Server error log.");
      }
   ...
   BackupExecuter::StartRestore(std::shared_ptr<Backup> pBackup)
   {
      ...
      String sXMLFile = sTempDir + "\\AltimailServerBackup.xml";
      FileUtilities::DeleteFile(sXMLFile);

      Compression oComp;
      if (!oComp.Uncompress(sZipFile, sTempDir, "AltimailServerBackup.xml"))
      {
         String sErrorMessage = Formatter::Format("Unable to uncompress AltimailServerBackup.xml from {0} to {1}. Please confirm that Altimail Server has permissions to {0} and {1}.", sZipFile, sTempDir);
      ...
      if (!pBackupNode)
      {
         String sErrorMessage = "The supplied XML file is not a valid Altimail Server backup file";
         Application::Instance()->GetBackupManager()->OnBackupFailed(sErrorMessage);
         return false;
      }
      ...
      if (iRestoreOptions & Backup::BODomains)
      {
         ...
         {
            String sErrorMessage = "Restore of domains failed. Please check Altimail Server log.";
      ...
      if (iRestoreOptions & Backup::BOSettings)
      {
         ...
         {
            String sErrorMessage = "Restore of settings failed. Please check Altimail Server log.";
            Logger::Instance()->LogBackup(sErrorMessage);
            Application::Instance()->GetBackupManager()->OnBackupFailed(sErrorMessage);
            return false;
         }
