...
namespace HM
{
   ...
   void SPFTester::Test()
   {
      ...
      if (SPF::Instance()->Test("102.222.22.87", "example@altimailserver.org", "mail.altimailserver.org", sExplanation) != SPF::Pass)
      {
         // Should be allowed. 
         throw;
      }

      if (SPF::Instance()->Test("1.2.3.4", "example@altimailserver.org", "mail.altimailserver.org", sExplanation) != SPF::Fail)
      {
