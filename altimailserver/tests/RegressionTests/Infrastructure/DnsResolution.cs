...
namespace RegressionTests.Infrastructure
{
   ...
   {
      private AltimailServer.Utilities _utilities;
      ...
      public void CNameRecordsShouldBeFollowed()
      {
         ...
         var actualServer = _utilities.GetMailServer("example@cname-test.AltimailServer.com");
         var expectedServer = _utilities.GetMailServer("example@AltimailServer.com");
      ...
      public void NoneExistantRecordsShouldNotResolve()
      {
         ...
         // According to RFC, a server owner should not add a CNAME record to MX record,
         // but many do and hMailServer has supported this historically.
         var actualServer = _utilities.GetMailServer("example@invalid.AltimailServer.com");
      ...
      public void InfinitelyRecursiveRecordsShouldNotResolve()
      {
         ...
         // According to RFC, a server owner should not add a CNAME record to MX record,
         // but many do and hMailServer has supported this historically.
         var actualServer = _utilities.GetMailServer("example@recursive1.AltimailServer.com");

         Assert.AreEqual("", actualServer);

         var errorLog = LogHandler.ReadAndDeleteErrorLog();

         StringAssert.Contains("Too many recursions during IP address lookup. Query: recursive2.AltimailServer.com", errorLog);
      }
      */
