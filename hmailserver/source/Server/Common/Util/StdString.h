...
//
//      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//      Credits to both Joe O'Leary and Szabolcs Szász for providing the original 
//      StdString.h and an updated version of StdString.h, respectively. 
// 
//      The majority of the code was good to go but some parts required some rework.
//      Some methods are missing from the updated version by Szabolcs and has 
//      been re-added as per the original StdString.h for hMailServer. 
//      Juan Davel, 2025-06-03
// 
//      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
//
//      (Look for "!!" markers in this file for each change I made.)
//      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//
//  REMARKS:
...
//
//	REVISION HISTORY
//   2025-MAY-29 - Originally forked from <https://github.com/xparq/CStdString> as 
//             a replacement for the existing hMailServer StdString.h file. This file 
//             is better compatible with C++ 17 which hMailServer has started using.
//             Some code were changed to make it compliant with hMailServer's needs, 
//             in addition to adding missing method/functions present in the old 
//             StdString.h file.
// 
...
#ifndef SS_WIN32
#ifndef MO_SS_WIN32
  //#if defined(_MSC_VER) || defined(__BORLANDC__) || defined(_WIN32) || defined(__MINGW32__) || defined(__MINGW64__) //HMS must be built with MSVC, no need to factor in MINGW
...
// This whole section was added, but was not present in the original StdString.h. We don't need it either because HMS MUST be compiled 
// using MSVC - we will not be compiling on MINGW32/MINGW64 and you shouldn't either! Obey the rules.
//#ifdef SS_NO_LOCALE
//
...
#ifdef SS_MBCS
      sDst.resize((size_t)sslen(szCvt));
#else
      sDst.resize(nDst);
      szCvt;
#endif
...
inline int	ssvsprintf(PSTR pA, size_t nCount, PCSTR pFmtA, va_list vl)
{
   return vsnprintf(pA, nCount, pFmtA, vl); //Fixed, Juan Davel, 2025/05/28
}
inline int	ssvsprintf(PWSTR pW, size_t nCount, PCWSTR pFmtW, va_list vl)
{
   return vswprintf(pW, nCount, pFmtW, vl); //Fixed, Juan Davel, 2025/05/28
}
...
inline int	ssvsprintf(PSTR pA, size_t nCount, PCSTR pFmtA, va_list vl)
{
   ...
   return vsnprintf_s(pA, nCount, nCount, pFmtA, vl);
#else
   return vsnprintf(pA, nCount, pFmtA, vl);
#endif
}
inline int	ssvsprintf(PWSTR pW, size_t nCount, PCWSTR pFmtW, va_list vl)
{
   ...
   return vswprintf_s(pW, nCount, pFmtW, vl);
#else
...
#else
template<typename CT>
//!!std::binary_function deprecated: https://stackoverflow.com/q/33114656/1479945
...
public:
   ...
   //#define SSRES(pctstr) LOWORD(static_cast<uintptr_t>(reinterpret_cast<const void*>(pctstr))) //quick x64 fix
#define SSRES(pctstr) LOWORD(reinterpret_cast<uintptr_t>(pctstr))
   ...
   bool TryLoad(const void* pT)
   {
      ...
      if ((pT != NULL) && SS_IS_INTRESOURCE(pT))
      {
         ...
         //UINT nId = LOWORD(static_cast<uintptr_t>(pT)); //64-bit appropriate
         UINT nId = LOWORD(reinterpret_cast<uintptr_t>(pT));
         if (!LoadString(nId))
         {
   ...
   MYTYPE& ToUpper(const std::locale& loc = std::locale())
   {
      ...
      std::transform(this->begin(),
         ...
         [&loc](auto c) -> CT { return SSToUpper<CT>()(c, loc); }); //!! () to help out handicapped compilers
#endif
...
#define STD_BUF_SIZE		1024
   ...
   //Re-added from original StdString.h. Juan, 29/05/2025
   bool StartsWith(PCMYSTR szText) const {
      int len = sslen(szText);

      if (Left(len).CompareNoCase(szText) == 0) return true;
      return false;
   }
   ...
   bool EndsWith(PCMYSTR szText) const {
      ...
      if (Right(len).CompareNoCase(szText) == 0) return true;
      return false;
   }
   ...
   int FindNoCase(PCMYSTR szSub) const
   {
      ...
      int nIdx = sFindIn.Find(sLookFor); //Change from MYSIZE to int
      return MYBASE::npos == nIdx ? -1 : nIdx;
   }
   ...
   int FindNoCase(PCMYSTR szSub, int nStart) const
   {
      ...
      int nIdx = sFindIn.Find(sLookFor, nStart); //Change from MYSIZE to int.
      return MYBASE::npos == nIdx ? -1 : nIdx;
   }
   ...
   int ReplaceNoCase(PCMYSTR szOld, PCMYSTR szNew)
   {
      ...
      if (0 != nOldLen)
      {
         ...
         if (nNewLen > nOldLen)
         {
            ...
            while (nIdx < this->length() &&
               ...
               (nIdx = this->FindNoCase(szOld, static_cast<int>(nIdx))) != MYBASE::npos)
            {
               nFound++;
               nIdx += nOldLen;
            }
      ...
      //return nReplaced;
      return static_cast<int>(nReplaced);
   }
